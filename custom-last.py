...
###### Preparation of Data / Batch Functions #######


def imageToNp (path):
    imgnp = cv2.imread(path)#.flatten()
    imgnp = cv2.resize(imgnp, (228, 228)).flatten() 
    return imgnp;
...
def maxpool2dnew(x, k):
    return tf.nn.max_pool(x, ksize=[1, k, k, 1], strides=[1, k, k, 1], padding='SAME')


###### Build Network Architecture ######### 
### 2 Conv. + MP Layers connected to FCN ###


def conv_net(x, weightsnew, biasesnew, dropout):
    # reshape image to 128x128 size
    x = tf.reshape(x, shape=[-1, 228, 228, 3])
    ...
    #conv2 = maxpool2dnew(conv2, k=2)
        ...
        # Convolution layer 2
    conv3 = conv2dnew(conv2, weightsnew['wc3'], biasesnew['bc3'])
    # Max pooling
    #conv3 = maxpool2dnew(conv3, k=1)

        # Convolution layer 2
    conv4 = conv2dnew(conv3, weightsnew['wc4'], biasesnew['bc4'])
    # Max pooling
    conv4 = maxpool2dnew(conv4, k=2)


    # Fully connected layer
    fc1 = tf.reshape(conv4, [-1, weightsnew['wd1'].get_shape().as_list()[0]])
    ...
    fc2 = tf.add(tf.matmul(fc1, weightsnew['wd2']), biasesnew['bd2'])
    # this is WX + b
    fc2 = tf.nn.relu(fc2)
    #fc2 = tf.nn.dropout(fc2, dropout)

    out = tf.add(tf.matmul(fc2, weightsnew['out']), biasesnew['out']) # output layer
    return out
	
 

######## Load Data from Pickles to Dataframe ###########
...
####### Initialize Network architecture and hyper-parameters #######
...
n_input = 155952 # 128x128x3 image
n_classes = 11 # 1 for each classification class [0-9]
dropout = 0.50 # experiment with multiple rates
...
weightsnew = {
    # conv filters: choosing 5x5 and 3 channels and 40 filters
    'wc1': tf.Variable(tf.random_normal([5, 5, 3, 40])),
    # filters for 2nd conv layer : 5x5 again but 20 filters
    # 3rd param is 10 because first conv layer output is 10 deep channels
    'wc2': tf.Variable(tf.random_normal([5, 5, 40, 60])),
    'wc3': tf.Variable(tf.random_normal([3, 3, 60, 80])),
    'wc4': tf.Variable(tf.random_normal([5, 5, 80, 100])),
    'wd1': tf.Variable(tf.random_normal([32*32*100, 4096])),
...
biasesnew = {
    ...
    'bc2': tf.Variable(tf.random_normal([60])),
